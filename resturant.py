def place_order(menu):
    ...
    while place_order == True:
        ...
        for menu_selection, options in menu_items.items():
            # print(f"Category {category} options {options}")
            # print(menu_items.keys())
            food_category, meal = options["Item name"].split(" - ")
            price = options["Price"]
            #print(f"{menu_selection}: {item['Item name']} - ${item['Price']:.2f}")
        ...
        if keep_ordering.lower() == "n":
            ...
            prices_list = [item["Price"] * item["Quantity"] 
                                for item in order]
            ...
            print(f"{order_total} order total")
...
def update_order(order, menu_selection, menu_items):
    ...
    if menu_selection.isdigit(): 
        ...
        if menu_selection in menu_items:
            ...
            quantity = input(f"What quantity of {item_name} would you? (This will default to 1 if number is not entered) ")
            ...
            if quantity.isdigit():
                ...
                print("Invalid input. Quantity has been set to the default value of 1.")
                quantity = 1
